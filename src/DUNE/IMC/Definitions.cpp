...
//***************************************************************************
// IMC XML MD5: dba16f8b87eb59249fd870a4bd286dc3                            *
//***************************************************************************
...
namespace DUNE
{
  namespace IMC
  {
    ...
    GroupEstimatedState::GroupEstimatedState(void)
    {
      m_header.mgid = 2020;
      clear();
      estate_list.setParent(this);
    }

    void
    GroupEstimatedState::clear(void)
    {
      estate_list.clear();
    }

    bool
    GroupEstimatedState::fieldsEqual(const Message& msg__) const
    {
      const IMC::GroupEstimatedState& other__ = static_cast<const GroupEstimatedState&>(msg__);
      if (estate_list != other__.estate_list) return false;
      return true;
    }

    int
    GroupEstimatedState::validate(void) const
    {
      return true;
    }

    uint8_t*
    GroupEstimatedState::serializeFields(uint8_t* bfr__) const
    {
      uint8_t* ptr__ = bfr__;
      ptr__ += estate_list.serialize(ptr__);
      return ptr__;
    }

    uint16_t
    GroupEstimatedState::deserializeFields(const uint8_t* bfr__, uint16_t size__)
    {
      const uint8_t* start__ = bfr__;
      bfr__ += estate_list.deserialize(bfr__, size__);
      return bfr__ - start__;
    }

    uint16_t
    GroupEstimatedState::reverseDeserializeFields(const uint8_t* bfr__, uint16_t size__)
    {
      const uint8_t* start__ = bfr__;
      bfr__ += estate_list.reverseDeserialize(bfr__, size__);
      return bfr__ - start__;
    }

    void
    GroupEstimatedState::fieldsToJSON(std::ostream& os__, unsigned nindent__) const
    {
      estate_list.toJSON(os__, "estate_list", nindent__);
    }

    void
    GroupEstimatedState::setTimeStampNested(double value__)
    {
      estate_list.setTimeStamp(value__);
    }

    void
    GroupEstimatedState::setSourceNested(uint16_t value__)
    {
      estate_list.setSource(value__);
    }

    void
    GroupEstimatedState::setSourceEntityNested(uint8_t value__)
    {
      estate_list.setSourceEntity(value__);
    }

    void
    GroupEstimatedState::setDestinationNested(uint16_t value__)
    {
      estate_list.setDestination(value__);
    }

    void
    GroupEstimatedState::setDestinationEntityNested(uint8_t value__)
    {
      estate_list.setDestinationEntity(value__);
    }
  }
}
