...
// Author: Josef Matous                                                     *
//***************************************************************************
// Utility to extract messages in CSV format from LSF files.                *
//***************************************************************************
...
inline void
write_trajectory_reference_header(std::ofstream& out, bool standalone)
{
  if (standalone)
    write_common_header(out);
...
write_header(std::string message_name, std::ofstream& out)
{
  ...
  if (message_name == "TrajectoryReference")
  {
    write_trajectory_reference_header(out, true);
    return;
  }
...
inline void
write_message(const DUNE::IMC::TrajectoryReference* msg, std::ofstream& out, bool standalone)
{
  if (standalone)
    write_timestamp_and_source(msg, out);

  out << msg->r_x << ", " << msg->r_y << ", " << msg->r_z << ", "
    << msg->v_x << ", " << msg->v_y << ", " << msg->v_z << ", "
    << msg->a_x << ", " << msg->a_y << ", " << msg->a_z << std::endl;
}
...
write_message(const DUNE::IMC::Message* msg, std::ofstream& out)
{
  ...
  if (msg->getId() == DUNE_IMC_TRAJECTORYREFERENCE)
  {
    write_message((const IMC::TrajectoryReference*) msg, out, true);
    return;
  }
