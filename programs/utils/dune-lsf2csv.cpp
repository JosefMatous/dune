...
inline void
write_consensus_packet_header(std::ofstream& out)
{
  write_common_header(out);
  out << "lat (rad), lon (rad), z (m), v_x (m/s), v_y (m/s), v_z (m/s)" << std::endl;
}

inline void
write_desiredspeed_header(std::ofstream& out)
{
  write_common_header(out);
  out << "value, speed units (enumerated)" << std::endl;
}
...
write_header(std::string message_name, std::ofstream& out)
{
  ...
  if (message_name == "ConsensusPacket")
  {
    write_consensus_packet_header(out);
    return;
  }

  if (message_name == "VirtualTarget")
  {
    write_consensus_packet_header(out);
    return;
  }

  if (message_name == "DesiredSpeed")
  {
    write_desiredspeed_header(out);
    return;
  }
...
inline void
write_message(const DUNE::IMC::ConsensusPacket* msg, std::ofstream& out)
{
  write_timestamp_and_source(msg, out);
...
inline void
write_message(const DUNE::IMC::VirtualTarget* msg, std::ofstream& out)
{
  write_message((const DUNE::IMC::ConsensusPacket*) msg->msg.get(), out);
}

inline void
write_message(const DUNE::IMC::DesiredSpeed* msg, std::ofstream& out)
{
  write_timestamp_and_source(msg, out);
...
write_message(const DUNE::IMC::Message* msg, std::ofstream& out)
{
  ...
  if (msg->getId() == DUNE_IMC_CONSENSUSPACKET)
  {
    write_message((const IMC::ConsensusPacket*) msg, out);
    return;
  }

  if (msg->getId() == DUNE_IMC_VIRTUALTARGET)
  {
    write_message((const IMC::VirtualTarget*) msg, out);
    return;
  }

  if (msg->getId() == DUNE_IMC_DESIREDSPEED)
  {
    write_message((const IMC::DesiredSpeed*) msg, out);
    return;
  }
}
